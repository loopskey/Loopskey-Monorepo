...
import { buildAssociationRejectionEmail } from "@mail/association-email.template";
...
@Injectable()
export class AssociationReviewNotificationService {
  private readonly logger = new Logger(
    AssociationReviewNotificationService.name,
  );
  ...
  async deliver(requestId: string, force = false) {
    ...
    try {
      ...
      this.logger.error("Association notification delivery failed", {
        requestId,
  ...
  private async buildApproval(request: {
    ...
    if (!request.approvedUserId)
      throw new ConflictException("Approved request has no Association user.");
    ...
    return buildAssociationActivationEmail({
      ...
      associationName: request.organizationName,
      supportEmail: this.requiredConfig("SUPPORT_EMAIL"),
      username: request.workEmail,
      activationUrl,
      loginUrl: this.requiredConfig("ASSOCIATION_LOGIN_URL"),
      expiresInMinutes,
    });
  }
  ...
  private buildRejection(request: {
    ...
    return buildAssociationRejectionEmail({
      appName: this.config.get<string>("APP_NAME", "LoopsKey"),
      associationName: request.organizationName,
      supportEmail: this.requiredConfig("SUPPORT_EMAIL"),
